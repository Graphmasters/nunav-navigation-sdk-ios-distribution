...
class RouteLineLayerHandler: MGLStyleLayersHandler {
    // MARK: Nested Types
    ...
    private(set) lazy var firstPartRouteLayer: MGLLineStyleLayer = RouteLineLayer(
        identifier: identifierPrefix + Constants.firstPartRouteLayerIdentifier, source: firstPartRouteSource
    )

    private(set) lazy var firstPartRouteOutlineLayer: MGLLineStyleLayer = RouteOutlineLayer(
        identifier: identifierPrefix + Constants.firstPartRouteOutlineLayerIdentifier, source: firstPartRouteSource
    )

    private(set) lazy var secondPartRouteLayer: MGLLineStyleLayer = RouteLineLayer(
        identifier: identifierPrefix + Constants.secondPartRouteLayerIdentifier,
        source: secondPartRouteSource
    )

    private(set) lazy var secondPartRouteOutlineLayer: MGLLineStyleLayer = RouteOutlineLayer(
    ...
    func refreshLayer(with fullRoute: [Route.Waypoint], forced: Bool = false) {
        if forced {
            cancelRemainingWork()
        }
