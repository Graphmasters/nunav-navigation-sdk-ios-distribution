...
class TripSymbolsLayerHandler: MGLStyleLayersHandler {
    // MARK: Nested Types
    ...
    private let durationFormatter: SingleDurationFormatter = HourSingleDurationFormatter()
    ...
    func refresh(origin _: LatLng? = nil, destinations: [LatLng]?, route: Route? = nil) {
    ...
    private func firstStopDuration(route: Route?) -> MGLPointFeature? {
        ...
        feature.attributes[DefaultIconLayer.textKey] = [
            ...
            FormattedDuration.localizedString(for: durationFormatter.formatSingle(duration: route.totalTravelTime))
        ].joined(separator: "\n")
        return feature
    }
